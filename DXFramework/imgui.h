...
namespace ImGui
{
    ...
    IMGUI_API void          SetWindowFontScale(float scale);                                    // per-window font scale_. Adjust IO.FontGlobalScale if you want to scale_ all windows
...
struct ImDrawData
{
    ...
    IMGUI_API void ScaleClipRects(const ImVec2& sc);  // Helper to scale_ the ClipRect field of each ImDrawCmd. Use if your final output buffer is at a different scale_ than ImGui expects, or if there is a difference between your window resolution and framebuffer resolution.
};
...
struct ImFont
{
    ...
    float                       Scale;              // = 1.f        // Base font scale_, multiplied by the per-window font scale_ which you can adjust with SetFontScale()
