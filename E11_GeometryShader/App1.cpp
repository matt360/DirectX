...
bool App1::render()
{
	...
	geometryShader->setShaderParameters(renderer->getDeviceContext(), worldMatrix, viewMatrix, projectionMatrix, textureMgr->getTexture("brick"));
	//// Render object (combination of mesh geometry and shader process
	geometryShader->render(renderer->getDeviceContext(), mesh->getIndexCount());
	
	// to show point data (hopefully)
	//// Send geometry data (from mesh)
...
void App1::gui()
{
	...
	//bool boo = false;
	//ImGui::Checkbox("Stuff", 0);
	ImGui::SliderFloat("float", &f, 0.0f, 1.0f);

	ImGui::SetNextWindowSize(ImVec2(200, 100), ImGuiSetCond_FirstUseEver);
	
	//ImGui::Begin("Another Window", &boo);
	ImGui::Begin("Framerate", 0, NULL);
	ImGui::Text("Hello");
	ImGui::End();
